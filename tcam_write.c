...
int32
sys_humber_aclqos_entry_delete(uint32 label_id, cm_aclqos_label_type_t label_type, cm_aclqos_key_type_t entry_type, uint32 entry_id)
{
	sys_aclqos_label_t *p_label;
	...
	for(lchip = 0; lchip < lchip_num; lchip++)
	{
		...
		_sys_acl_get_sys_entry_by_eid(entry_id, &p_entry);
		if(!p_entry)
		{
			return CM_E_ACLQOS_ENTRY_NOT_EXIST;
		}
	...
	CM_ERROR_RETURN(_sys_humber_aclqos_entry_remove(pb, p_entry));
...
int32
sys_humber_aclqos_entry_delete_all(uint32 label_id, cm_aclqos_label_type_t label_type, cm_aclqos_key_type_t entry_type)
{
	sys_aclqos_label_t *p_label;
	cm_list_pointer_t *p_list;
	cm_list_pointer_node_t *p_node, *p_next_node;
	...
	for(lchip = 0; lchip < lchip_num; lchip++)
	{
		...
		{
			p_entry = _cm_container_of(p_node, sys_aclqos_entry_t, head);
...
int32
sys_humber_aclqos_entry_action_add(uint32 label_id, cm_aclqos_label_type_t label_type, cm_aclqos_key_type_t entry_type, uint32 entry_id, cm_aclqos_action_t *p_action)
{
	sys_aclqos_label_t *p_label;
	sys_aclqos_entry_t *p_entry;
	uint8 lchip, lchip_num;
	...
	for(lchip = 0; lchip < lchip_num; lchip++)
	{
		...
		if(p_action->flag & CTC_ACLQOS_ACTION_FLOW_POLICER_FLAG)
		{
			...
			CM_ERROR_RETURN(sys_humber_qos_flow_policer_bind(lchip, p_action->policer_id));	
			CM_ERROR_RETURN(sus_humber_qos_policer_index_get(lchip, p_action->policer_id, &policer_ptr));
